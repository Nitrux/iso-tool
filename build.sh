...
curl -X PUT --url https://storage.bunnycdn.com/isoreleases/$image.md5sum -H "AccessKey: $BUNNY_API_KEY" -H 'Content-Type: application/octet-stream' --data-binary "~@$output_dir/${image%.iso}.md5sum"

curl -X PUT --url https://storage.bunnycdn.com/isoreleases/$image.zsync -H "AccessKey: $BUNNY_API_KEY" -H 'Content-Type: application/octet-stream' --data-binary "~@$output_dir/${image%.iso}.zsync"
