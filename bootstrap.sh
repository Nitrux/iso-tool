...
'
apt update
apt install -y apt-transport-https wget ca-certificates gnupg2
...
if echo ee86878b3be00f5c99da50974ee7c5141a163d0e00fccb889398f1a33e112584 neon.key | sha256sum -c; then
	...
	echo '##################' >> /etc/apt/sources.list.d/neon.list
	echo '# KDE Neon Repos #' >> /etc/apt/sources.list.d/neon.list
	echo '##################' >> /etc/apt/sources.list.d/neon.list
	echo '' >> /etc/apt/sources.list.d/neon.list
	echo '# deb http://archive.neon.kde.org/dev/unstable/ bionic main' >> /etc/apt/sources.list.d/neon.list
	echo 'deb http://archive.neon.kde.org/dev/stable/ bionic main' >> /etc/apt/sources.list.d/neon.list
...
if echo b51f77c43f28b48b14a4e06479c01afba4e54c37dc6eb6ae7f51c5751929fccc nxos.key | sha256sum -c; then
	...
	echo '################' >> /etc/apt/sources.list.d/nxos.list
	echo '# Nitrux Repos #' >> /etc/apt/sources.list.d/nxos.list
	echo '################' >> /etc/apt/sources.list.d/nxos.list
	echo 'deb http://repo.nxos.org/stable/ nxos main' >> /etc/apt/sources.list.d/nxos.list
	echo 'deb http://repo.nxos.org/development/ nxos main' >> /etc/apt/sources.list.d/nxos.list
	echo 'deb http://repo.nxos.org/testing/ nxos main' >> /etc/apt/sources.list.d/nxos.list
	echo '' >> /etc/apt/sources.list.d/nxos.list
fi
rm nxos.key

apt update
apt install -y nomad-desktop
